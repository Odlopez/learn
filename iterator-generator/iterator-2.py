...
class Multifilter:
    ...
    # Проверяем элемент на соответствие нашим правилам
    def check(self, element):
        pos = 0
        neg = 0

        # Проходим по всем проверочным функциям и накапливаем "совпадения" и "провалы"
        for it in self.funcs:
            ans = it(element)
            pos += 1 if ans else 0
            neg += 0 if ans else 1

        # возвращаем результат окончательной проверки у решающей функции
        return self.judge(pos, neg)

    # запоминаем введенные при инициализации данные
    def __init__(self, iterable, *funcs, judge=judge_any):
        self.funcs = list(funcs)
        self.iterable = iterable
        self.judge = judge

    # создаем генератор, который возвращает через yield только
    # проходящие проверку элементы
    def __iter__(self):
